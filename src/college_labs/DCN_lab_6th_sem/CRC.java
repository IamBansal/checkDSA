...
public class CRC {
    ...
    static String Mod2Div(String dividend, String divisor) {
        ...
        while (pick < n) {
            if (tmp.charAt(0) == '1') tmp = Xor(divisor, tmp) + dividend.charAt(pick);
            else tmp = Xor(new String(new char[pick]).replace("\0", "0"), tmp) + dividend.charAt(pick);
            pick += 1;
        }
    ...
    static void Receiver(String data, String key) {
        ...
        while (curr != data.length()) {
            if (currxor.length() != key.length()) currxor += data.charAt(curr++);
            else currxor = Mod2Div(currxor, key);
        }
        if (currxor.length() == key.length()) currxor = Mod2Div(currxor, key);
        if (currxor.contains("1")) System.out.println("there is some error in data");
        else System.out.println("correct message received");
    }

    public static void main(String[] args) {
        String data = "11001001";
        String key = "1001";
        System.out.println("\nSender side...");
        EncodeData(data, key);
